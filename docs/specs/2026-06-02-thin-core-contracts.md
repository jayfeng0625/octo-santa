...
**Delivery-timing variance is isolated behind the harness `settle`/`advance` seam (the VARIANT, per the keystone above).** The suite settles at every assertion point through ONE uniform `settle(harness)` primitive: push impls (the InMemory reference, synchronous in-`publish` fan-out) leave `harness.advance` undefined → `settle` microtask-drains; poll impls (the SQLite adapter, future Cloudflare) define `advance()` as a single deterministic, TIMER-FREE `pump()` tick, and the at-least-once redelivery "next cycle" is that tick (§2.7: redelivery = "next publish (push) or next poll tick (poll)"; CLAUDE.md: cross-process delivery requires polling). This is the **α** form — a deterministic tick, no sleeps/timers — ratified R6 (#2662); the **β** time-tolerant form (`until(pred, {timeoutMs})`) was NOT adopted, so no flake enters. The asserted BEHAVIORS (cross-peer delivery, FIFO, catch-up, redelivery, HOL, duplicate-tolerance) and the publish/subscribe sequences are the INVARIANT — byte-identical across backends; only the `settle`/`advance` timing primitive re-parameterizes. There is intentionally no delivery-timing descriptor axis (descriptor locked to 4 axes per §1 D "no speculative third"); the timing variance lives in the harness seam, not the descriptor.
