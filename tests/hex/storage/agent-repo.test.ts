...
describe("SqliteAgentRepo", () => {
  ...
  it("listAll returns all agents except _system", () => {
    const { db, repo } = setup();
    repo.register("agent-a", process.pid);
    const now = Date.now();
    db.run(
      `INSERT INTO agents (id, created_at, last_seen_at, pid, registered_at)
       VALUES ('agent-b', ${now}, ${now}, NULL, NULL)`
    );
    const all = repo.listAll();
    expect(all.length).toBe(2);
    expect(all.map((a) => a.id).sort()).toEqual(["agent-a", "agent-b"]);
    db.close();
  });
  ...
  it("heartbeatOrReclaim reclaims a stale owner", () => {
    const { db, repo } = setup();
    repo.register("test-agent", process.pid);
    db.run("UPDATE agents SET pid = 999999, last_seen_at = 0 WHERE id = ?", ["test-agent"]);
    expect(repo.heartbeatOrReclaim("test-agent", process.pid)).toBe("ok");
    expect(repo.findById("test-agent")!.pid).toBe(process.pid);
    db.close();
  });
});
